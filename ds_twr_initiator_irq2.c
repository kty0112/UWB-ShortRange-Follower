/*
 * @file    ds_twr_initiator_irq2.c
 * @brief   DS-TWR initiator with Token-Passing (SLAVE)
 *
 * Init_2 is the slave node: boots into WAIT_TURN mode.
 * Waits for TURN from Init_1, then does Res_1 → Res_2 → TURN → wait.
 * If Init_1 is offline, 50ms timeout triggers self-start.
 */
...
#define APP_NAME "DS TWR INIT IRQ v2.0 (Token-Passing SLAVE)"
...
/* DS-TWR 3-way 메시지 버퍼 */
static uint8_t tx_poll_msg[]  = { 0x41, 0x88, 0, 0xCA, 0xDE, 0x01, 0x00, 'V', 'E', 0x21 };
...
int ds_twr_initiator_irq(void)
{
    ...
    raspi_send((unsigned char *)"8. ALL SETUP OK! Waiting for TURN...\r\n");
